...
ub=5*power(10,p_order);
lb=-5*power(10,p_order);
...
rd_score_his=zeros([1,search_times]);
...
for each_iter=1:search_times
    ...
    a_data=ifft(fft(data,[],2).*exp(2j*pi*(fc+refe_scale).*y/3e8),[],2);
    a_data=a_data(:,501:end-500);
    [~,entro_his,~]=Normal_pga(a_data,15);

    score_his(each_iter)=a_score;
    posi_his(:,each_iter)=a_wolf;
    rd_score_his(each_iter)=min(entro_his);
    disp("iter："+string(each_iter)+"。a："+string(a)+"。score："+string(a_score)+"。 rd_score："+string(min(entro_his))+"。wolfs："+string(a_wolf));

end     % end for each_iter
toc

fit_y=load("..\MEARP_RDA\fit_y.mat").fit_y;
find_y_win=load("..\CCEA_RDA\find_y.mat").find_y;
find_y_min=load("..\MEARP_RDA\find_y.mat").find_y;
tmp_data_win_=ifft(fft(data,[],2).*exp(2j*pi*(fc+refe_scale).*find_y_win/3e8),[],2);
tmp_data_min_=ifft(fft(data,[],2).*exp(2j*pi*(fc+refe_scale).*find_y_min/3e8),[],2);


tmp_data_win=mean(abs(tmp_data_win_),1);
tmp_data_min=mean(abs(tmp_data_min_),1);
tmp_entropy_win=-sum((tmp_data_win/sum(tmp_data_win)).*log2(tmp_data_win/sum(tmp_data_win)));
tmp_entropy_min=-sum((tmp_data_min/sum(tmp_data_min)).*log2(tmp_data_min/sum(tmp_data_min)));
tmp_data_win_=tmp_data_win_(:,501:end-500);
tmp_data_min_=tmp_data_min_(:,501:end-500);
[~,entro_win,~]=Normal_pga(tmp_data_win_,15);
[~,entro_min,~]=Normal_pga(tmp_data_min_,15);

scla=ones(1,search_times);

figure;
f_size=18;
subplot(121)
plot(1:search_times,score_his,1:search_times,tmp_entropy_win*scla,1:search_times,tmp_entropy_min*scla)
set(gca,fontname="Times New Roman",fontsize=f_size)
grid on;

subplot(122)
plot(1:search_times,rd_score_his,1:search_times,min(entro_win)*scla,1:search_times,min(entro_min)*scla)
set(gca,fontname="Times New Roman",fontsize=f_size)
grid on;
...
save("rd_score_his.mat","rd_score_his")
